...
export default function LineChart5() {
  ...
  return (
    <div className=" flex items-center justify-center p-0">
      <Card className="w-full max-w-3xl rounded bg-card shadow-none">
        <CardHeader className="border-0 min-h-auto pt-6 pb-4">
          <CardTitle className="text-lg font-semibold">Vente de cette produit</CardTitle>
          ...
          {Array(4).fill(null).map((_, index) => (
            <Avatar key={index} className='ring-background ring-2 transition-all duration-300 ease-in-out'>
              <AvatarImage src={"https://cdn.shadcnstudio.com/ss-assets/avatar/avatar-2.png"} alt={`Avatar ${index + 1}`} />
              <AvatarFallback className='text-xs'>{`A${index + 1}`}</AvatarFallback>
            </Avatar>
          ))}
        </div>
          <CardToolbar>
          ...
          <div className="flex items-center flex-wrap gap-3.5 md:gap-10 px-5 mb-8 text-sm">
            <div className="flex items-center gap-3.5">
              ...
              <div className="flex items-center gap-2">
                ...
                <Badge variant={salesChange >= 0 ? 'default' : 'destructive'} >
